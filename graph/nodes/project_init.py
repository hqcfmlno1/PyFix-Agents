...
class ProjectInitializerNode(BaseNode[BugFixState]):
    ...
    async def run(self, ctx: GraphRunContext[BugFixState]) -> "InitialInputRouterNode":
        from graph.nodes.initial_input_router import InitialInputRouterNode
        ...
        if ctx.state.preset_repo_path:
            abs_path = os.path.abspath(ctx.state.preset_repo_path)
            if not os.path.isdir(abs_path):
                raise FileNotFoundError(f"Thư mục dự án không tồn tại: {abs_path}")
            ctx.state.repo_path = abs_path
            ctx.state.is_repo_valid = True
            print(f"  {GREEN}✓ Đã nhận dự án (auto): {abs_path}{RESET}")
        else:
            while True:
